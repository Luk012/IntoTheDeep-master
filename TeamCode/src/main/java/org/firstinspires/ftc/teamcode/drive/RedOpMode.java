...
public class RedOpMode extends LinearOpMode
{
    ...
    public String Retrun_Color(ColorSensor colorSensor)
    {
       ...
       int  green_value = colorSensor.green();
        ...
        if(red_value > green_value && red_value > blue_value && red_value > 150)
        {
            return "red";

        }
        else if(green_value > blue_value && green_value > 150)
        {
           return "yellow";
        }
        else if(blue_value > 150)
        {
...
//        outtake.update(r,claw,lift,fourbar,clawAngle,transfer);
        ...
        while (opModeIsActive() && !isStopRequested())
        {
            ...
            if(!previousGamepad1.touchpad && currentGamepad1.touchpad)
            {
                ...
                transfer.CS = transferController.transferStatus.TRANSFER_DONE;
            }
            ...
            if(collectAngle.CS == collectAngleController.collectAngleStatus.COLLECT)
            {
              ...
              if(Objects.equals(Retrun_Color(r.color_sensor), "blue"))
                {
                    can_transfer = false;
                }
              else {
...
//            }
                ...
                if(can_outtake == true) {
                    if (!previousGamepad2.circle && currentGamepad2.circle) {
                        if (outtake.CS == outtakeController.outtakeStatus.SPECIMEN_HIGH) {
                            ...
                            outtake.CS = outtakeController.outtakeStatus.INTER;
                        } else {
                    ...
                    if (!previousGamepad2.square && currentGamepad2.square) {
                        ...
                        else outtake.CS = outtakeController.outtakeStatus.INTER;
                    }
            ...
            if(!previousGamepad1.right_bumper && currentGamepad1.right_bumper)
            {
                if(extendo.CS != extendoController.extendoStatus.EXTENDED)
                {
                    ...
                    if(org.firstinspires.ftc.teamcode.Globals.globals.is_intransfer)
                    {
                        transfer.CS = transferController.transferStatus.TRANSFER_DONE;
                ...
                {
                    if(has_sample == false && collectAngle.CS == collectAngleController.collectAngleStatus.DRIVE)
                    {
                        extendo.CS = extendoController.extendoStatus.RETRACTED;
                    }else
