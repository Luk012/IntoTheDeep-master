...
public class PidControllerTest extends LinearOpMode {
    ...
    public static double RetractedPosition = 0 , ExtendedPosition = 20000;
    ...
    public void runOpMode() throws InterruptedException {
        ...
        {
            MotorConfigurationType motorConfigurationType = robot.lift.getMotorType().clone();
            motorConfigurationType.setAchieveableMaxRPMFraction(1.0);
            robot.lift.setMotorType(motorConfigurationType);

            int ColectarePosition = robot.collect.getCurrentPosition();
            double powerColectare = hello.update(ColectarePosition);
            powerColectare = Math.max(-1,Math.min(powerColectare,1));
            robot.lift.setPower(powerColectare);
